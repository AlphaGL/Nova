...
def home(request):
    ...
    room_messages = Message.objects.filter(Q(room__topic__name__icontains=q)).order_by('-id')
...
def room(request, pk):
    ...
    if request.method == 'POST':
        media = request.FILES.get('media')  # if using <input name="media" />
...
def topics_page(request):
    return render(request, 'study_groups/topics.html', {})